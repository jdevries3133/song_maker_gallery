...
class BaseCase(StaticLiveServerTestCase):
    ...
    def setUpClass(cls):
        ...
        if settings.INTEGRATION_TEST_BROWSER == "CHROME":
            ...
            raise ValueError(
                "Value for INTEGRATION_TEST_BROWSER was not valid. "
    ...
    def submit_login_form(self, username: str, password: str) -> None:
        ...
        """
        self.first_el(self.await_data_testid("usernameInput")).send_keys(username)
        self.first_el(self.await_data_testid("passwordInput")).send_keys(password)
    ...
    def create_sample_gallery(self):
        ...
        with open(
            Path(Path(__file__).parent, "mock_data", "gallery.json"), "r"
        ) as jsonf:
            ...
            if serializer.is_valid():
                ...
                self.fail(f"Serializer not valid due to errors: {serializer.errors}")
    ...
    def await_xpath(
        self, xpath: str, timeout: int = 3, many: bool = False
    ...
    def await_id(
        self, id_: str, timeout: int = 3, many: bool = False
    ) -> Union[WebElement, List[WebElement]]:
        """
        Wraps await_xpath
        """
    ...
    def await_data_testid(
        self, test_id: str, timeout: int = 3, many: bool = False
    ) -> Union[WebElement, List[WebElement]]:
        """
        Wraps await_xpath
        """
    ...
    def await_text(
        self, text: str, timeout: int = 3, many: bool = False
    ) -> Union[WebElement, List[WebElement]]:
        return self.await_xpath(f'//*[text() = "{text}"]', timeout, many)

    def first_el(self, item: Union[WebElement, List[WebElement]]) -> WebElement:
        """
    ...
    def all_el(self, item: Union[WebElement, List[WebElement]]) -> List[WebElement]:
        """
